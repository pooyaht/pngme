...
impl Png {
    ...
    pub fn from_file<P: AsRef<Path>>(path: P) -> Result<Self> {
        ...
        file.read_to_end(&mut bytes)
            .map_err(|_| anyhow!("couldn't read the file"))?;
    ...
    pub fn remove_chunk(&mut self, chunk_type: &str) -> Result<Chunk> {
        ...
        let chunk_position = iter
            .position(|c| c.chunk_type().to_string() == chunk_type)
            .ok_or_else(|| anyhow!("chunk type {} not found", chunk_type))?;
    ...
    pub fn chunk_by_type(&self, chunk_type: &str) -> Option<&Chunk> {
        let mut iter = self.chunks.iter();
        iter.find(|c| c.chunk_type().to_string() == chunk_type)
    }
    ...
    pub fn as_bytes(&self) -> Vec<u8> {
        ...
        let mut bytes = Vec::new();
        bytes.append(Self::STANDARD_HEADER.to_vec().as_mut());
        bytes.append(chunk_bytes.as_mut());
        bytes
    }
}
...
impl TryFrom<&[u8]> for Png {
    ...
    fn try_from(bytes: &[u8]) -> Result<Png> {
        let header = bytes.get(0..8).ok_or(anyhow!("Invalid Header Length"))?;
        if header != Self::STANDARD_HEADER {
            return Err(anyhow!("Invalid Header"));
        };

        let mut chunks_bytes = bytes.get(8..).ok_or(anyhow!("There are no chunks"))?;
        let mut chunks: Vec<Chunk> = Vec::new();
        while !chunks_bytes.is_empty() {
            let chunk = Chunk::try_from(chunks_bytes)?;
            let chunk_length = 12 + chunk.length() as usize;
...
impl Display for Png {
    fn fmt(&self, f: &mut Formatter<'_>) -> Result<(), std::fmt::Error> {
        let chunks_as_strings: Vec<String> = self.chunks.iter().map(|c| c.to_string()).collect();
        write!(
            f,
            "Header: {}\nChunks: {}",
            Self::STANDARD_HEADER
